...
function validatePlan(raw, { examScope, field, dailyHours, studyDays, offDay }) {
  if (!raw || typeof raw !== 'object' || !Array.isArray(raw.days)) throw new Error('invalid_plan_json');
  const allowed = new Set(allowedSubjects(examScope, field).map((x) => x.toLocaleLowerCase('tr-TR')));
  ...
  for (const dayRaw of raw.days) {
    ...
    for (const session of sessionsRaw) {
      ...
      const topic = clean(session?.topic ?? session?.task, 220) || 'Konu tekrarı + soru çözümü';
      ...
      const allowedMatch = allowed.has(subjectKey) || allowed.has(`${examType} ${subject}`.toLocaleLowerCase('tr-TR'));
...
function promptFor({ examScope, field, dailyHours, studyDays, offDay, intensity, weakSubjects }) {
  const subjects = allowedSubjects(examScope, field);
  return [
    ...
    `Yoğunluk: ${intensity}.`,
    `Zorlanılan dersler: ${weakSubjects.length ? weakSubjects.join(', ') : 'belirtilmedi'}.`,
...
export const generateWeeklyStudyPlanHandler = onCall(
  ...
  async (request) => {
    ...
    const input = { examScope, field, dailyHours, studyDays, offDay, intensity, weakSubjects };
